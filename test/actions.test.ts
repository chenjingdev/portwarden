...
describe('listener stop scope', () => {
  it('shares a stop scope only with the same PID, never other jobs in the same PGID', () => {
    const selected = listener({pid: 1001, pgid: 9000, collectorPgid: 8000});
    expect(listenerSharesStopScope(selected, listener({pid: 1001, port: 3001}))).toBe(true);
    expect(listenerSharesStopScope(selected, listener({pid: 1002, pgid: 9000}))).toBe(false);
  });
});

describe('stopListener safety', () => {
  it('does not report the PID stopped while one of its other ports remains open', async () => {
    vi.useFakeTimers();
    const current = listener();
    const sibling = listener({pid: current.pid, port: 3001});
    const config = repository();
    const actions = new PortwardenActions(config, {
      collect: sequentialCollector([current, sibling], [sibling]), kill: vi.fn<Kill>(),
    });
  ...
  it.each(['SIGTERM', 'SIGKILL'] as const)('sends %s only to the selected PID and preserves a pinned sibling job sharing its PGID', async (signal) => {
    const selected = listener({pid: 1000, pgid: 9000, collectorPgid: 8000});
    const sibling = listener({pid: 1001, ppid: 9000, pgid: 9000, port: 3001, args: 'vite --port 3001'});
    const config = repository({pinnedListenerKeys: [listenerKey(sibling)]});
    const collect = sequentialCollector([selected, sibling], [sibling]);
    const kill = vi.fn<Kill>();
    const actions = new PortwardenActions(config, {collect, kill});
    const outcome = await actions.stopListener(selected, signal);
    expect(kill.mock.calls).toEqual([[selected.pid, signal]]);
    expect(outcome.pgid).toBeUndefined();
    expect(config.get().pinnedListenerKeys).toContain(listenerKey(sibling));
  });

  it.each(['app', 'system', 'dev'] as const)('never signals a process group for a %s listener', async (kind) => {
    const current = listener({kind, pgid: 9000, collectorPgid: 8000});
    const kill = vi.fn<Kill>();
    const actions = new PortwardenActions(repository(), {collect: sequentialCollector([current], []), kill});
    await actions.stopListener(current, 'SIGKILL');
    expect(kill.mock.calls).toEqual([[current.pid, 'SIGKILL']]);
  });
