...
function toggleSelectedEntryPinned(state) {
  ...
  const nextPinnedKeys = entryPinned
    ? pinnedKeys.filter((key) => key !== entryKey)
    : [...pinnedKeys.filter((key) => key !== entryKey), entryKey];
  ...
  persistEntryPreferences(
    state,
    nextConfig,
    entryPinned ? `Unpinned: ${entry.port}` : `Pinned: ${entry.port}`,
    getSelectionKey(entry),
...
function moveSelectedEntryToNextPort(state, helpers) {
  ...
  setImmediate(() => {
    try {
      ...
      pruneRevivablePinsForMovedEntry(state, entry);
...
function pruneRevivablePinsForMovedEntry(state, entry) {
  const pins = state.config?.revivablePins;
  if (!pins || typeof pins !== "object") return;
  const keys = getEntryListenerKeys(entry).filter((key) => key && Object.prototype.hasOwnProperty.call(pins, key));
  if (keys.length === 0) return;
  const nextRevivablePins = { ...pins };
  for (const key of keys) delete nextRevivablePins[key];
  const nextConfig = { ...state.config, revivablePins: nextRevivablePins };
  try {
    saveConfig(nextConfig);
    state.config = nextConfig;
  } catch (error) {
    state.error = `Graveyard 정리 실패: ${error.message}`;
  }
}
...
function buildGraveyardFooter(columns) {
  return dim(truncateDisplay("r revive   d drop   g/s/esc/q 닫기", columns));
}
...
module.exports = {
  ...
  __testing: {
    ...
    moveSelectedEntryToNextPort,
    openGraveyardView,
    closeGraveyardView,
    pruneRevivablePinsForMovedEntry,
