...
function refreshTuiData(state, reason = "manual", preferredSelectionKey = null) {
  try {
    ...
    state.visibleListeners = selectListeners(allListeners, { all: state.listScope === "all" }, state.config);
...
function isEntryPinnedWithConfig(config, entry) {
  ...
  return pinnedListenerKeys.includes(listenerKey);
}
...
function getVisiblePinnedEntryKeys(state, config = state.config) {
  return uniqueOrderedValues(
    state.visibleListeners
      .filter((entry) => isEntryPinnedWithConfig(config, entry))
...
function rebuildPinnedListenerKeys(state, nextPinnedVisibleListenerKeys, listenerKeyToRemove = "") {
  const visibleKeySet = new Set(uniqueOrderedValues(state.visibleListeners.map((entry) => getEntryListenerKey(entry))));
...
function buildMainHeaderLine(state, totalVisible, hiddenCount) {
  const badges = [
    ...
    buildBadge(`${totalVisible} port${totalVisible === 1 ? "" : "s"}`, "bright"),
  ];
...
function buildSelectedSummaryLines(state, selected, columns) {
  ...
  return [
    ...
    truncateDisplay(`next ${nextPort}  dup ${duplicateCount > 1 ? `${duplicateCount} in use` : "none"}  host ${selected.host}`, columns),
...
function buildTuiRow(entry, columns, showKind, isSelected, pinLabel = "-") {
  ...
  const cells = [];
  if (showKind) {
    cells.push(padDisplay(String(entry.kind).toUpperCase(), widths.kind));
  }
  ...
  cells.push(padDisplay(entry.displayProject || "-", widths.project));
