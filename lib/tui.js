...
const { clamp, colorize, dim, formatTimestamp, pad, padDisplay, renderShortcutLine, truncate, truncateDisplay } = require("./utils");
...
function refreshTuiData(state, reason = "manual", preferredSelectionKey = null) {
  try {
    ...
    state.visibleListeners = selectListeners(allListeners, { all: false });
...
function renderMainScreen(state, columns, rows) {
  ...
  if (visibleRows.length === 0) {
    ...
    for (let index = 0; index < visibleRows.length; index += 1) {
      ...
      lines.push(buildTuiRow(entry, columns, false, isSelected));
    }
  }
  ...
  if (selected) {
    ...
    lines.push(`dup  ${duplicateCount > 1 ? `${duplicateCount} in use` : "none"}  next ${nextPort}`);
...
function buildTuiHeader(columns, showKind) {
  ...
  const cells = [];
  if (showKind) {
    cells.push(pad("KIND", widths.kind));
  }
  cells.push(pad("PORT", widths.port));
  cells.push(pad("PID", widths.pid));
  cells.push(pad("AGE", widths.age));
  cells.push(pad("HOST", widths.host));
  cells.push(pad("PROCESS", widths.process));
  return `  ${cells.join(" ")}`;
}

function buildTuiRow(entry, columns, showKind, isSelected) {
  const widths = getTuiColumnWidths(columns, showKind);
  const cells = [];
  if (showKind) {
    cells.push(pad(entry.kind, widths.kind));
  }
  cells.push(pad(String(entry.port), widths.port));
  cells.push(pad(String(entry.pid), widths.pid));
  cells.push(pad(entry.elapsed, widths.age));
  cells.push(pad(entry.host, widths.host));
  cells.push(pad(entry.displayCommand, widths.process));
...
function getTuiColumnWidths(columns, showKind) {
  ...
  const fixed = widths.port + widths.pid + widths.age + widths.host + (showKind ? widths.kind : 0);
  const spaces = showKind ? 6 : 5;
