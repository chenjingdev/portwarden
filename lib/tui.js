...
function renderMainScreen(state, columns, rows) {
  ...
  const scopeLabel = state.listScope === "all" ? colorize("[ALL]", "cyan") : colorize("[MAIN]", "green");
  const countsLabel =
    state.listScope === "all"
      ? `${totalVisible} LISTEN port${totalVisible === 1 ? "" : "s"}`
      : `${totalVisible} pinned/dev port${totalVisible === 1 ? "" : "s"}`;
  const hiddenLabel = state.listScope === "all" || hiddenCount === 0 ? "" : `  hidden ${hiddenCount}`;
  ...
  lines.push(colorize("portwarden", "bright") + `  ${scopeLabel}  ${countsLabel}${hiddenLabel}${busyLabel}`);
  lines.push("");
  ...
  if (visibleRows.length === 0) {
    ...
    for (let index = 0; index < visibleRows.length; index += 1) {
      ...
      lines.push(buildTuiRow(entry, columns, showKind, isSelected, isEntryPinned(state, entry)));
    }
  }
