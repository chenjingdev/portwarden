...
function collectAllListeners() {
  ...
  return collapseEquivalentListeners(
    baseListeners
...
function selectListeners(allListeners, options, config = null) {
  ...
  const filtered = allListeners.filter(
    (entry) =>
      entry.kind === "dev" ||
      isEntryPinnedBySet(entry, pinnedListenerKeySet)
  );
...
function launchDetachedCommand(command, cwd) {
  ...
  // stale cwd / missing $SHELL would otherwise emit an async 'error' event that kills the TUI.
  child.on("error", () => {});
  child.unref();
...
function buildListenerKey(host, port) {
  const hostKey = normalizeWhitespace(host || "");
  const portKey = String(port || "-").trim();
  return `host:${hostKey || "-"}::port:${portKey || "-"}`;
}

function getEntryListenerKeys(entry) {
  return unique([entry?.host || "", ...(Array.isArray(entry?.listenerHosts) ? entry.listenerHosts : [])])
    .map((host) => buildListenerKey(host, entry?.port));
}

function getEntryListenerKey(entry) {
  return getEntryListenerKeys(entry)[0] || buildListenerKey("", entry?.port);
}

function entryMatchesListenerKey(entry, listenerKey) {
  const normalizedKey = String(listenerKey || "").trim();
  if (!normalizedKey) {
    return false;
  }

  return getEntryListenerKeys(entry).includes(normalizedKey);
}

function isEntryPinnedBySet(entry, pinnedListenerKeySet) {
  return getEntryListenerKeys(entry).some((listenerKey) => pinnedListenerKeySet.has(listenerKey));
}

function getEntryOrderIndex(entry, orderedIndex) {
  let bestIndex = Number.MAX_SAFE_INTEGER;

  for (const listenerKey of getEntryListenerKeys(entry)) {
    if (orderedIndex.has(listenerKey)) {
      bestIndex = Math.min(bestIndex, orderedIndex.get(listenerKey));
    }
  }
...
function sortListeners(entries, config = null) {
  ...
  return baseline.sort((left, right) => {
    const leftListenerPinned = isEntryPinnedBySet(left, pinnedListenerKeySet);
    const rightListenerPinned = isEntryPinnedBySet(right, pinnedListenerKeySet);
    ...
    const leftOrder = getEntryOrderIndex(left, orderedIndex);
    const rightOrder = getEntryOrderIndex(right, orderedIndex);
...
function collapseEquivalentListeners(entries) {
  const collapsedEntries = [];
  const entriesByKey = new Map();

  for (const entry of entries) {
    const key = `${entry.pid}:${entry.port}:${normalizeWhitespace(entry.displayHost || entry.host || "")}`;
    const existingEntry = entriesByKey.get(key);

    if (!existingEntry) {
      const nextEntry = {
        ...entry,
        listenerHosts: unique([entry.host, ...(Array.isArray(entry.listenerHosts) ? entry.listenerHosts : [])]),
      };
      entriesByKey.set(key, nextEntry);
      collapsedEntries.push(nextEntry);
      continue;
    }

    existingEntry.listenerHosts = unique([
      ...existingEntry.listenerHosts,
      entry.host,
      ...(Array.isArray(entry.listenerHosts) ? entry.listenerHosts : []),
    ]);
  }
...
function captureReviveSnapshot(entry) {
  if (!entry) return null;
  const cmd = String(entry.args || "").trim();
  if (!cmd) return null;
  const listenerKey = getEntryListenerKey(entry);
  if (!listenerKey) return null;
  return {
    listenerKey,
    record: {
...
function buildReviveSlug(listenerKey, record) {
  const portMatch = String(listenerKey || "").match(/::port:([0-9]+)/);
  const port = portMatch ? portMatch[1] : "0";
  const cwdHint = String(record?.cwd || "")
    .split(/[\\/]+/)
    .filter(Boolean)
    .pop() || "";
  const raw = cwdHint ? `${cwdHint}-${port}` : `port-${port}`;
  const sanitized = raw.toLowerCase().replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "");
  return sanitized || `port-${port}`;
}

function buildReviveLogPath(pin) {
  if (!pin || typeof pin !== "object") {
    throw new Error("buildReviveLogPath requires a pin with listenerKey + record.");
  }
  const listenerKey = pin.listenerKey;
  const record = pin.record || pin;
  const slug = buildReviveSlug(listenerKey, record);
  return path.join(HOME, ".portwarden", "logs", `${slug}.log`);
}

function reviveSnapshot(pin, logPath) {
  if (!pin || typeof pin !== "object" || !pin.record) {
    throw new Error("reviveSnapshot requires { listenerKey, record }.");
  }
  const { record } = pin;
  const shell = process.env.SHELL || "/bin/zsh";
  const cmd = String(record.cmd || "").trim();
  if (!cmd) throw new Error("Revive cmd is empty.");
  const cwd = String(record.cwd || "").trim();
  const shellCommand = cwd ? `cd ${shellEscape(cwd)} && ${cmd}` : cmd;

  if (process.env.DEV_PORTS_SPAWN_DRY_RUN === "1") {
    return { pid: null, command: shellCommand, logPath };
  }
  ...
  try {
    ...
    // stale cwd / missing $SHELL would surface as uncaughtException and kill the TUI.
    child.on("error", (err) => {
      try {
        fs.appendFileSync(logPath, `\n[portwarden revive error] ${err && err.message ? err.message : err}\n`);
      } catch (_) {}
    });
    child.unref();
    return { pid: child.pid || null, command: shellCommand, logPath };
  } finally {
