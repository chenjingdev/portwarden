...
function replaceHomeSegments(input, home) {
  if (!home) {
    return String(input || "");
  }

  return String(input || "").split(home).join("~");
}

function extractPackageNameFromPath(input) {
  ...
  if (segments[0].startsWith("@")) {
    return segments[1] ? `${segments[0]}/${segments[1]}` : "";
  }

  return segments[0];
}

function parsePnpmDlxCommand(input) {
  const cleaned = normalizeWhitespace(input);
  if (!cleaned) {
    return null;
  }

  const pnpmDlxPattern = /(?:^|[\\/])Library[\\/]Caches?[\\/]pnpm[\\/]dlx[\\/]/;
  const tokens = cleaned.split(" ");
  for (let index = 0; index < tokens.length; index += 1) {
    const token = stripOuterQuotes(tokens[index]);
    const normalizedToken = token.replace(/[\\/]+/g, "/");
    if (!pnpmDlxPattern.test(normalizedToken)) {
      continue;
    }

    const packageName = extractPackageNameFromPath(normalizedToken);
    if (!packageName) {
      continue;
    }
    ...
    return {
      packageName,
      scriptPath: token,
      trailingArgs: tokens.slice(index + 1).join(" "),
    };
  }
...
function summarizeCommand(command, args, home) {
  ...
  const pnpmDlxCommand = parsePnpmDlxCommand(cleanedArgs);
  if (pnpmDlxCommand) {
    const trailingArgs = replaceHomeSegments(pnpmDlxCommand.trailingArgs, home);
    return `pnpm dlx ${pnpmDlxCommand.packageName}${trailingArgs ? ` ${trailingArgs}` : ""}`;
  }

  return replaceHomeSegments(cleanedArgs, home);
}
...
function formatHostDisplay(input) {
