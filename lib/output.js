...
function renderOnce(listeners, allListenerCount, options, isWatch = false) {
  ...
  const scopeLabel = options.all ? "All LISTEN ports" : "Pinned + dev ports";
  ...
  if (listeners.length === 0) {
    console.log(options.all ? "No LISTEN ports found." : "No pinned or dev-like LISTEN ports found.");
    return;
  }

  const table = buildTable(listeners, options.all || listeners.some((entry) => entry.kind !== "dev"));
  console.log(table);
}
