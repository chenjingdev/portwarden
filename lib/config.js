...
function normalizeRevivablePinRecord(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  if (typeof value.cwd !== "string") return null;
  if (typeof value.cmd !== "string" || value.cmd.trim().length === 0) return null;
  if (typeof value.capturedAt !== "string" || value.capturedAt.length === 0) return null;
...
function normalizeRevivablePins(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  const result = {};
  for (const key of Object.keys(value)) {
    const normalizedKey = String(key || "").trim();
    if (!normalizedKey) continue;
    const record = normalizeRevivablePinRecord(value[key]);
    if (record) result[normalizedKey] = record;
  }
  return result;
}
...
function normalizeOrderedKeyList(value) {
  return normalizeStringList(value)
    .map((item) => {
      if (item.startsWith("listener:")) {
        return item.slice("listener:".length);
      }
...
function normalizeConfig(config) {
  return {
    ...
    pinnedListenerKeys: normalizeStringList(config?.pinnedListenerKeys),
    orderedEntryKeys: normalizeOrderedKeyList(config?.orderedEntryKeys),
    revivablePins: normalizeRevivablePins(config?.revivablePins),
  };
}
