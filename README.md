...
- 실행 파일 경로만 끝내 확인되지 않는 경우 `x`는 중단하지만, 명시적인 강제 종료 `f`는 같은 사용자·PID·포트·시작 시각·cwd·명령이 직전 스냅샷과 모두 일치하면 진행합니다.
- 검증된 개발 서버의 외부 프로세스 그룹은 `x`에서 `SIGTERM`, `f`에서 `SIGKILL`로 함께 정리합니다. 앱·시스템 리스너, PortWarden 자신의 그룹, 소유자나 전체 구성원을 확인할 수 없는 그룹은 선택한 PID만 종료하고 경고합니다.
- 핀한 리스너는 `x`, `f`, `--kill-*`로 종료할 수 없습니다. 같은 종료 대상 그룹에 핀된 포트가 하나라도 있으면 그룹 전체를 보호합니다.
...
-b, --browser <name>      이번 TUI에서 사용할 브라우저
    ...
    --kill-port <port>    포트의 리스너 종료(검증된 개발 서버 그룹 포함)
    --kill-pid <pid>      리스너 종료 범위 또는 감지된 좀비 PID 종료
```
